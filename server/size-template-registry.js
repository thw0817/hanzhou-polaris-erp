...
function normalize(input, existing = null) {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    throw new TypeError("尺寸模板数据格式不正确");
  }
  ...
  if (!name) throw new TypeError("尺寸模板名称必填");
  if (!storeId) throw new TypeError("尺寸模板必须绑定授权店铺");
  if (!categoryId || !productTypeId) {
    throw new TypeError("尺寸模板必须绑定SHEIN末级类目");
  }
  if (!rows.length) throw new TypeError("尺寸模板至少需要一个SKU尺寸");
  ...
  return {
    ...
    categoryId,
    productTypeId,
    shape: input.shape === "round" ? "round" : "rectangle",
    rows,
...
export class SizeTemplateRegistry {
  #templates = new Map();

  constructor({ filePath }) {
    if (!filePath) throw new TypeError("Size template registry filePath is required");
  ...
  save(input) {
    ...
    if (input.id && !existing) {
      const error = new Error("未找到要更新的尺寸模板");
      error.status = 404;
      throw error;
    }
  ...
  #load() {
    ...
    if (envelope.version !== FILE_VERSION || !Array.isArray(envelope.templates)) {
      throw new Error("Unsupported SHEIN size template registry format");
    }
