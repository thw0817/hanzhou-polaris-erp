...
以上来源只证明官方页面公开了这些字段。它们不证明当前授权店铺可调用、不证明当前凭证有效，也不证明线上 adapter 可以放行；因此 3 项的 `authorizedStoreRead` 仍必须是 `not_observed`，字段 `observed` 必须是 `false`。
...
这两项修正只改变请求契约的准确性，不会打开 ERP-07 远端读取开关，也不代表已完成授权店铺实读。

## 仍然阻断的接口

以下 3 项当前没有足够的独立官方完整 response 原文或店铺只读回执，继续保留 `internal_consumer_contract` 与 `official_response_fields_not_captured`：

- `sales.sku`
- `preflight.publish_quota`
- `review.document_state`

特别是当前代码使用的 `/open-api/goods-publish-quotas/detail` 没有在本次归档中找到可确认的独立官方响应字段，不能把旧的上架额度接口字段映射到新路径。

## 结论

本记录只完成 ERP-07 的来源证据增量，不代表 ERP-07 完成，不代表 ERP-06 可以解除 `BLOCKED/NO-GO`，不代表可以部署或执行任何真实 SHEIN 写入。后续仍需真实授权店铺只读回执、统一 adapter 受控接线、预发 canary/readback 和单独部署批准。
