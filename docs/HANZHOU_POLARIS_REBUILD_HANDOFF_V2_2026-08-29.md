...
当前执行：ERP-05 行级关系证据 Run 已完成允许范围但完成门阻断；ERP-06～ERP-23 不得开始。
...
ERP-00～ERP-23 不是历史已执行步骤；当前已由用户明确启动并完成 ERP-00～ERP-04，ERP-05 的原始只读历史证据审计和前序生产补证均因逐条证据缺口阻断，当前行级关系 Run 已完成允许范围内检查，但仍因完整对象证据、9 条官方 version 不匹配、新模型逐条映射和 SKU 应用角色可读证据缺失而阻断。后续步骤仍只有在前一步完成且用户明确要求后，才读取执行计划和台账并创建 Run。
...
- `BLOCKED`：ERP-05（行级关系 Run 已完成允许范围内检查；目标关系孤儿为 0，但完整对象证据、9 条官方 version 不匹配、新模型逐条映射和 SKU 应用角色可读证据仍缺失；前序阻断记录保留）。
- `NOT_STARTED`：ERP-06～ERP-23。
- 当前正式 ERP Run：`RUN-20260829-ERP05-ROW-RELATION-FINGERPRINT-07`；前序 Run 作为阻断历史保留。当前 Run 已核对 50 个声明外键、目标表行数/字段缺口/孤儿关系和零写入证据，但未完成新模型逐条映射。
