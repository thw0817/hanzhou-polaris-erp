...
> **重要校正：** ERP-00～ERP-23 是在 17 个板块方案完成后编制的工程实施路线，不是旧“第 1～20 步”的改名版。历史旧步骤与本路线仍然分开；本计划的当前执行状态以执行台账为准。2026-08-29 已取得用户明确启动，ERP-00～ERP-04 已完成，ERP-05 Run 08、Run 09、Run 10、Run 11 和修正 AWS4 签名后的 Run 13 的 S3/AWS4 兼容列表请求均被 HTTP 403 阻断；Run 11/13 已核对服务器掩码密钥与 `wow-rug-cos-service` API 密钥前缀/后缀一致，且权限页显示 `polaris-media-list-readonly` 为直接关联；Run 14 已用 COS 原生 HMAC-SHA1 成功取得完整列表并完成媒体归属对账。当前 633 个对象与数据库媒体记录匹配，187 个无远端对象且均无引用。用户已明确批准 COS-first：新系统以 COS 为文件主存储，数据库保留元数据与业务引用；历史 ProductVersion/PublishAttempt/PlatformProductLink 映射冻结为只读 legacy，不作为新链路进入 ERP-06 的前置条件。ERP-06 仅按本计划的非生产模型设计与验证边界启动。
...
- 用户批准的“历史映射冻结豁免”属于已记录的范围调整，不等于伪造历史完成；旧记录仍保留为只读证据，ERP-06 只能建设新链路，不能借此修改旧数据。
...
- 100% 可取得的历史记录进入一个证据分类或明确 `UNKNOWN`；用户批准历史映射冻结豁免后，未能安全映射的记录保持 `legacy_unversioned`/`UNKNOWN` 只读，不再作为 ERP-06 新链路的前置条件。
- 没有执行 UPDATE、DELETE、重试或清队列。
- ERP-20 的修复范围可以由报告精确指定，或明确记录为不在本轮范围内。

### 12.1 ERP-05 历史映射冻结豁免（已批准）

2026-08-29 用户明确批准：采用 COS-first；历史数据冻结只读，新系统以 COS 为文件主存储，数据库保留元数据与业务引用，允许 ERP-05 历史映射豁免后进入 ERP-06。

执行边界：

1. 633 个已核验 COS 对象可作为新系统的远端资产来源；187 条无远端对象且无业务引用的历史媒体记录保留原状，禁止自动删除、恢复、重试或改状态。
2. 旧 ProductVersion/PublishAttempt/PlatformProductLink 无法安全建立的关系不伪造、不强行合并；历史证据仍由 ERP-05 报告留档。
3. ERP-06 只建立新的规范数据模型、事件账本、COS-first 媒体引用和 additive migration；不执行生产数据库迁移，不重写旧记录。
4. 后续新业务必须遵守“COS 对象先成功并完成完整性核验，数据库再登记元数据/引用”的顺序。
...
### 13.2 生产等价预发准入包（2026-08-31）

`RUN-20260831-ERP06-PREPRODUCTION-PACKET-24` 已把后续预发所需的变更记录、候选制品、备份恢复、047/048 专属检查、最小权限、关闭开关、观察和回滚证据收敛为单一无敏感模板。它不创建预发环境、不迁移、不授权、不启动 Worker，也不解除 `BLOCKED/NO-GO`。现有 staging 只有基础依赖而没有获准候选 Control/Worker 或恢复点，不能直接复用；047/048 在进入经评审的活动 migration 目录前更不得执行。详见 [ERP06_PRODUCTION_EQUIVALENT_PREPRODUCTION_ADMISSION_2026-08-31.md](./ERP06_PRODUCTION_EQUIVALENT_PREPRODUCTION_ADMISSION_2026-08-31.md)。
...
- Redis 是传输工具，不是业务真相。
- 所有迁移只做前向兼容，不重写已执行迁移。
- 新媒体文件以 COS 为本体；数据库保存可审计元数据、业务引用、版本所有权和保留/删除事件，不把数据库孤立行当作文件存在性证明。
