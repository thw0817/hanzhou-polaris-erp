...
- 未闭合门：本机未安装 Docker，且用户授权后通过 Homebrew 两次安装 Docker Desktop 均因官方 DMG 下载连接被重置失败，无法做 staging 实际启动/迁移/隔离验证；当前没有 Outbox Dispatcher，manifest 固定标记 `not_implemented` 并阻断新 PublishCommand；Playwright bundled Chromium 下载失败，但系统 Chrome 实测已通过，CI 默认 bundled Chromium 路径仍需在 CI runner 验证。
