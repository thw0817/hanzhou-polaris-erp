...
| BUG-MEDIA-016 | P1 | 当前 S3-compatible AWS4 `ListObjectsV2` 探针在 Tencent COS 返回 403，但 COS 原生 HMAC-SHA1 列表成功；若继续复用 AWS4 列表协议，会把协议不兼容误判成权限失败 | OPEN_REVERIFY | ERP-05、07、15、19、21 | COS 原生签名/官方 SDK 的 provider adapter、分页 fixture、403 分类和禁止错误 fallback；不能用单一 AWS4 结果判断 COS 权限 |
| BUG-MEDIA-017 | P1 | `media_assets` 仍保留 187 条无远端对象历史记录（deleted 185、failed 2），虽然均无引用，但缺少经过证据门的 tombstone/失败处置分类 | OPEN_REVERIFY | ERP-05、06、15、19、20、21 | 明确历史删除与失败上传语义、保留不可变审计、人工批准的修复/清理命令和零误删回归；当前不得自动删除、恢复或重试 |
...
- 固化探针首次运行：发现 PostgreSQL `42601` prepared statement 不允许包含多条 INSERT/DELETE；事务已回滚。修复为同一事务内逐条参数化 SQL，避免把测试工具自身错误混入业务结论。
...
- 结果：数据库 15 个核心表集合指纹和 6 个关系集合指纹已取得；当前快照包含 Draft 93、Batch 32、BatchItem 250、Job 219、Run 28、Receipt 157、Review 156、SPU 518、SKC 547、SKU 0、MediaAsset/Reference 820/542、Webhook 2,978、SyncJob/Item 571/339。逐条指纹只输出单向哈希，不暴露原始身份。
- 结果：媒体只读 `HEAD` 820 条中成功 585、404 173、超时 62；成功项大小/类型不匹配均为 0；未下载、上传、删除或复制对象。
- 结果：当前生产未发现 ProductVersion 或 PublishAttempt 专用表；`shein_authorization_attempts` 21 条属于授权流程 Attempt；Job `readback` JSON 219 条仅为本地字段，不能证明官方回读。
- 结果：61 个 Draft 被多个 Job 复用（最多 6 个）、12 个 running Run 全部超过 1 小时、2 个 claim 已过期、82 个 submitted Job 缺 completed_at、13 个 queued Webhook 已有 processed_at、媒体引用计数不一致 280 条；外键孤儿为 0。
- 完成门结论：`BLOCKED`；允许范围内的只读补证已经结束，但 ProductVersion/PublishAttempt/PlatformProductLink 逐条映射、完整对象存储清单和 SHEIN 官方回读仍缺失；不得开始 ERP-06、不得执行 ERP-20 修复。
- 当前状态：`BLOCKED`；报告见 `docs/ERP05_HISTORICAL_DATA_EVIDENCE_REPORT_2026-08-29.md`，完整证据时间和单向指纹均已登记。

### RUN-20260829-ERP05-OBJECT-READBACK-EVIDENCE-04

- 类型：ERP-05 缺口证据补证；媒体 provider-level 只读核验与本地回执/回读结构索引。
- 启动依据：用户继续明确要求“下一步”；上一 Run 已完成允许范围内检查但因对象清单与官方回读证据不足而阻断。
- 目标：在不改变任何生产状态的前提下，按媒体数据库状态分层复核对象 `HEAD` 的 404/超时/成功结果；仅统计发布回执、Job 回读字段的存在性、类型和不可逆结构指纹，确认哪些是本地记录、哪些仍没有官方证据。
- 允许范围：非交互 SSH；PostgreSQL 只读 `SELECT`/系统目录；使用已确认的 `S3ObjectStorage.statObject()` 发送 `HEAD`，不下载对象；输出状态/错误类别/计数/单向哈希；读取容器与日志元数据，不读取 payload。
...
- 启动依据：Run 08 第 1 页 HTTP 403；用户已在目标 COS 子用户上绑定限定存储桶的 `cos:GetBucket` 策略。
- 允许范围：非交互 SSH；现有生产环境变量；PostgreSQL `SELECT`/系统统计；对象存储 `ListObjectsV2` 分页请求；内存计数、大小、分页完整性和单向 key 摘要。
- 禁止范围：对象下载/上传/删除/复制/改名；数据库写入/迁移；SHEIN API、Redis/队列、部署、重启、切换和敏感值输出。
- 当前状态：`BLOCKED`；生产只读探针已执行，第 0 页 HTTP 403；失败关闭，不自动重试。

### RUN-20260829-ERP05-OBJECT-INVENTORY-RECHECK-09 结果

- 执行时间：2026-08-29 20:53:23～20:56:51（Asia/Shanghai）；生产只读。
- Provider：当前生产容器配置发起 `ListObjectsV2`，第 0 页 HTTP 403；未取得清单，不输出原始 key，未发生对象或数据库写入。
...
- 允许范围：非交互 SSH；生产容器现有环境变量；PostgreSQL `SELECT`/系统统计；对象存储单次 `ListObjectsV2` 分页请求；仅输出计数、大小和完成标志。
- 禁止范围：对象或数据库任何写入；SHEIN API、Redis/队列、部署、重启、切换、敏感值和原始对象键输出。
- 当前状态：`BLOCKED`；第 0 页仍 HTTP 403，失败关闭，不自动重试。

### RUN-20260829-ERP05-OBJECT-INVENTORY-RECHECK-11 结果

- 执行时间：2026-08-29 21:21:04 左右（Asia/Shanghai）；生产只读。
- Provider：第 0 页 HTTP 403；`pages=0`、`objects=0`、`bytes=0`、`complete=false`；未发生对象下载、上传、删除、复制或改名。
...
- 既有异常：官方 version mismatch 继续沿用 `total=9/crossUnique=0/crossNone=9/crossAmbiguous=0`，分类 `UNKNOWN`，不回读、不重发。
- 完成门结论：`BLOCKED`；用户提供的服务器掩码前缀/后缀与 `wow-rug-cos-service` API 密钥截图一致，且权限页已显示 `polaris-media-list-readonly` 为直接关联；但 `ListObjectsV2` 仍 HTTP 403，仍需核对该策略最终 JSON 已保存生效；ERP-06、ERP-20 和媒体清理不得开始。
...
- 类型：ERP-05 COS 原生签名与 S3/AWS4 兼容签名分流的只读验证。
- 启动依据：Run 11/13 仍 HTTP 403；Run 13 已修正 AWS4 canonical query 但未改变结果。
- 允许范围：非交互 SSH；生产容器现有环境变量；COS XML API 原生 HMAC-SHA1 `GET Bucket`；PostgreSQL `SELECT`/系统统计；仅输出脱敏状态、错误分类、计数和大小。
- 禁止范围：对象/数据库任何写入；SHEIN API、Redis/队列、部署、重启、切换、策略修改、敏感值和原始 object key 输出。
- 完成标准：成功则分页到 `IsTruncated=false` 并完成总数/大小对账；失败则保留 `UNKNOWN/BLOCKED`，并记录 `AccessDenied`、签名不匹配、网络或其他脱敏错误。
...
- COS 原生列表：COS XML API 原生 HMAC-SHA1 `GET Bucket` 成功；1 页、633 个唯一对象、总大小 `949039963` bytes、`IsTruncated=false`、桶内孤儿 0、重复 0。
- 数据库对账：`media_assets` 820 行、820 个唯一 `object_key`；633 条与 COS 对象精确匹配，187 条无远端对象；缺失记录为 `deleted=185`、`failed=2`，实际引用数为 0，`reference_count>0` 为 0；缺失累计记录大小 `228526673` bytes。
...
## 33. ERP-06 真实 sender → 官方回读边界隔离实现

### RUN-20260830-ERP06-SHEIN-REMOTE-BOUNDARY-13

- 类型：ERP-06 当前隔离实现单元；实现 Worker 到真实 SHEIN sender、官方单据状态/SPU 回读之间的严格契约，不触碰生产。
- 启动依据：第 32 Run 已完成隔离 Worker 编排；本 Run 补齐远端边界的请求构造、显式授权、默认禁网、回读证据完整性和失败回归，但不把它接入生产 Worker。
- 允许范围：新增隔离 remote boundary、复用现有官方 body/endpoint 和安全 projection、注入 fake request/credential resolver、失败回归、全量门禁和文档台账更新。
- 禁止范围：生产 PostgreSQL/COS/Redis/队列/SHEIN 访问或写入；正式 migration、生产部署/重启/切换；真实凭证解析、真实签名请求、Webhook 持久化、历史回填、自动重试或真实发布。
- 实际文件：[erp06-shein-remote-boundary.js](../server/cloud/erp06-shein-remote-boundary.js)、[erp06-shein-remote-boundary.test.js](../server/cloud/erp06-shein-remote-boundary.test.js)。未修改旧 `product-publish-worker`、生产路由或 `server/cloud/migrations/`。
- 契约边界：发布只接受 `erp06-publish-command-v1` 与既有 `/open-api/goods/product/publishOrEdit`，要求 Attempt=`claimed` 和显式 publishing authorization；回读固定为 `/open-api/goods/query-document-state`、`/open-api/goods/spu-info`，分别使用 `version + spuList:[{spuName}]`、`languageList + spuName`。
- 失败保护：所有身份/scope/版本指纹/path/body 漂移在凭证解析前拒绝；默认 `executionEnabled=false`、`readbackEnabled=false`；上游 status/code/traceId 保留且不自动重试；空、部分、版本不一致或 malformed 回读不解除 `result_unknown`。
- 本地验证：新边界回归 `9/9`；组合定向回归 `40/40`；全量测试 `1275/1275`；服务端测试 `125/125`；秘密扫描 `scannedFiles=627, findings=[]`；`npm run build:v2`、release audit、`node --check`、`git diff --check` 均通过。
...
## 35. ERP-06 单阶段官方回读编排隔离实现

### RUN-20260830-ERP06-OFFICIAL-READBACK-ORCHESTRATION-15

- 类型：ERP-06 当前隔离实现单元；把 remote boundary 与官方 readback repository 编排为一次明确的单阶段操作，不触碰生产。
- 启动依据：第 34 Run 已完成官方回读事实落账；本 Run 补齐 `document_state`/`spu_info` 的单阶段调用、关闭态 no-op、scope/版本指纹校验和一次性落账边界，仍不接入生产 Worker。
- 允许范围：新增隔离编排服务、单阶段输入/结果契约、readback repository 调用串联、失败回归、全量门禁和文档台账更新。
- 禁止范围：生产 PostgreSQL/COS/Redis/队列/SHEIN 访问或写入；真实凭证解析、生产 Worker/路由接线、正式 migration、生产部署/重启/切换；自动重试、自动重发、历史回填、平台 Link 自动创建或 `completed` 伪造。
- 实际文件：[erp06-official-readback-orchestrator.js](../server/cloud/erp06-official-readback-orchestrator.js)、[erp06-official-readback-orchestrator.test.js](../server/cloud/erp06-official-readback-orchestrator.test.js)。未修改生产 Worker、生产路由、`server/cloud/migrations/` 或现有 staging。
- 编排边界：一次操作只能明确选择 `document_state` 或 `spu_info`，只调用对应的官方回读 endpoint；关闭态只返回 disabled，不调用凭证、网络或 repository；成功 `read` 结果才向 repository 传递一次安全 projection。
- 失败保护：队列任务、tenant/store、Command/Attempt/ProductVersion、版本指纹、stage/endpoint、dry-run projection 和 `read` 状态漂移均 fail closed；repository 异常原样上抛，不自动重试；空回读不触发第二阶段，不解除 `result_unknown`。
- 本地验证：新编排回归 `8/8`；组合定向回归 `68/68`；全量测试 `1291/1291`；服务端测试 `125/125`；秘密扫描 `scannedFiles=631, findings=[]`；`npm run build:v2`、release audit、`node --check`、`git diff --check` 均通过。
...
## 43. ERP-07 response evidence 与消费者字段失败边界

### RUN-20260830-ERP07-RESPONSE-EVIDENCE-05

- 类型：ERP-07 第五段本地隔离实现；把剩余 6 个 source-pending 可执行 endpoint 的内部消费者字段显式登记为 response evidence，并为全量 33 个 endpoint 增加证据清单完整性门禁。
- 实际文件：[erp07-shein-endpoint-schema.js](../server/cloud/erp07-shein-endpoint-schema.js)、[erp07-shein-endpoint-schema.test.js](../server/cloud/erp07-shein-endpoint-schema.test.js)、[erp07-shein-adapter.js](../server/cloud/erp07-shein-adapter.js)。本轮仍未接入线上路由、Worker、生产配置、真实 SHEIN HTTP 或真实授权店铺读取。
...
## 47. ERP-07 diagnostics 未知 metadata fail-closed 修正

### RUN-20260830-ERP07-RESPONSE-EVIDENCE-DIAGNOSTICS-09

- 类型：ERP-07 第九段本地安全修正；收紧脱敏响应证据捕获器的 `diagnostics` 输入边界，不触碰生产。
- 失败基线：捕获器原先只拒绝敏感字段名，未知的 `diagnostics.message` 等扩展 metadata 仍可能被接受后静默丢弃；这会使未审计输入进入证据边界，且可能夹带凭证或原始上游信息。
- 允许范围：仅允许 `diagnostics.status`、`diagnostics.code`、`diagnostics.traceId`、`diagnostics.durationMs`；继续递归拒绝敏感键；保留原有成功摘要、字段观察和人工审阅状态语义。
- 禁止范围：不改变 endpoint catalog、官方 response provenance、authorized-store evidence 状态、adapter 接线、网页、数据库、COS、Redis、队列、migration 或任何生产配置；不解析真实凭证，不发送真实 SHEIN HTTP。
...
### RUN-20260830-LEDGER-STATE-CONSISTENCY-11

- 类型：执行台账与交接文档一致性修正；不改变业务代码、数据库、COS、Redis、SHEIN adapter 或任何部署行为。
- 失败基线：步骤总览同时把 ERP-06 与 ERP-07 标为 `IN_PROGRESS`，但顶层当前活动步骤只有 ERP-07，违反“同一时间只能一个步骤为 `IN_PROGRESS`”的硬规则。
- 最小修正：ERP-06 的生产接入前置审查明确为 `BLOCKED/NO-GO`，其本地隔离实现完成但生产迁移、真实 sender/Worker、正式回读接线和部署仍等待单独批准；ERP-07 保持当前唯一 `IN_PROGRESS`，当前 Run 为 `RUN-20260830-ERP07-RESPONSE-EVIDENCE-STATUS-CONSISTENCY-10`。
- 防回归：新增 `server/ci/erp-ledger-consistency.test.js`，强制校验恰好一个 `IN_PROGRESS`、步骤行与顶层 current run 一致，且 current run 必须指向最新 Run 记录。测试先以 `2 !== 1` 复现失败，文档修正后应通过。
- 同步范围：执行台账、V2 交接文档和主执行计划同步记录 ERP-06/ERP-07 的真实状态边界，避免把“生产接入被阻断”与“本地隔离工作仍在进行”混成两个并行活动步骤。
- 环境边界：只读文档审计与本地回归；未读取/打印真实凭证，未发送 SHEIN HTTP，未访问或写入生产/现有 staging PostgreSQL、COS、Redis、队列，未执行 migration、部署、重启、配置切换、历史回填或自动重发。
- 当前状态：`COMPLETE / DOCUMENTATION CONSISTENCY CORRECTION`；ERP-07 仍是唯一 `IN_PROGRESS`，ERP-06 为 `BLOCKED/NO-GO`，ERP-08～ERP-23 未开始。

## 50. ERP-07 来源引用完整性防回归

### RUN-20260830-ERP07-SOURCE-REFERENCE-INTEGRITY-12

- 类型：ERP-07 本地证据账本防回归；把 endpoint 来源文件、response evidence 来源文件及可选行号引用固化为自动门禁，不触碰生产。
...
## 51. ERP-07 证据捕获入口与范围未知字段 fail-closed 修正

### RUN-20260830-ERP07-EVIDENCE-INPUT-BOUNDARY-13

- 类型：ERP-07 本地安全边界修正；拒绝证据捕获入口和 `scope` 中未声明的扩展字段，防止凭证、回执元数据或未审计内容被静默丢弃，不触碰生产。
- 失败基线：捕获器原先会严格拒绝 `response`/`diagnostics` 的未知字段，但入口参数和 `scope` 的未知字段可能被函数解构或字段投影静默忽略；调用方无法确认这些字段是否经过审计，敏感内容可能进入未定义的捕获边界。
- 实际修正：[erp07-response-evidence.js](../server/cloud/erp07-response-evidence.js) 现在只接受 `endpoint/scope/sourceRef/observedAt/response` 五类入口字段，`scope` 只接受 `tenantId/storeId/supplierId`；未知入口字段和范围字段直接 fail closed。响应摘要仍固定为 `pending_manual_acceptance`，不升级官方字段或 `authorizedStoreRead`。
- 失败回归：[erp07-response-evidence.test.js](../server/cloud/erp07-response-evidence.test.js) 新增未知入口扩展字段和敏感 `scope` 字段拒绝测试；原有敏感 diagnostics、未知 metadata、非法 response、非只读 endpoint 和失败状态回归继续通过。
- 本地验证：证据捕获/ERP-07 schema 定向回归 `25/25`；全量 `npm test` `1365/1365`；V2 构建（1953 modules）、工具链（Node `24.16.0`、npm `11.13.0`、lockfile `3`）、密钥扫描（`scannedFiles=649, findings=[]`）、静态 release audit（`READY`，15/15）、staging isolation（14/14）和干净 release manifest 均通过。
- 证据语义：本修正只收紧输入边界，不补写官方 response 字段、不创建真实授权店铺证据、不改变 6 项 source-pending 状态。
...
## 53. ERP-07 source-pending 响应证据审阅摘要

### RUN-20260830-ERP07-SOURCE-PENDING-DOSSIER-15

- 类型：ERP-07 本地脱敏证据审阅边界；为 `sales.sku`、`preflight.publish_quota`、`review.document_state` 三项仍待核验的只读接口固定 endpoint 身份，不触碰生产。
- 失败基线：既有证据快照只保存 endpoint 名称和字段出现性；审核人无法只根据摘要复核调用对应的精确 method/path，存在把另一接口结构相近响应误作本接口候选证据的风险。
- 实际变更：[erp07-response-evidence.js](../server/cloud/erp07-response-evidence.js)、[erp07-response-evidence.test.js](../server/cloud/erp07-response-evidence.test.js) 与 [ERP07_SOURCE_PENDING_EVIDENCE_DOSSIER_2026-08-30.md](./ERP07_SOURCE_PENDING_EVIDENCE_DOSSIER_2026-08-30.md)。新 `buildErp07ResponseEvidenceReviewDossier()` 只接受既有脱敏快照，按当前 schema 重新派生 `method/path`，只输出 scope SHA-256、响应 SHA-256、字段覆盖与固定拒绝结论。
- Fail-closed：非 source-pending 只读接口、非待人工审核快照、contract/schema 版本漂移、端点与字段清单不一致、观测次数/类型摘要自相矛盾、未知 dossier 字段均稳定拒绝。调用方不能传入或覆盖 method/path；方法路径只能由已注册 schema 派生。
- 证据语义：无论候选响应是否覆盖全部字段，摘要一律输出 `catalogUpgrade.status=blocked_source_pending` 与 `eligible=false`；它不保存原始 payload、字段值、scope 原值、请求、headers、凭证、签名 URL、文件或图片，不改变 response evidence catalog 或 `authorizedStoreRead`。
- 本地验证：证据捕获/审阅摘要及 ERP-07 相邻定向回归 `60/60` 通过。尚未执行全量测试、构建、候选制品或 deployment；它们必须在干净最终 revision 上重新运行，不能把该定向结果冒充 release 证据。
...
## 55. ERP-07 网页商品单据状态 source-pending 绕过收口

### RUN-20260830-ERP07-WEB-DOCUMENT-STATE-GUARD-17

- 类型：ERP-07 本地隔离 fail-closed 修正；收口受保护网页路由 `POST /v1/web/stores/:storeId/publish/document-state` 的服务层远端回读绕过，不触碰生产。
- 失败基线：`server/cloud/web-business-service.js` 的 `queryDocumentState()` 虽被认证、可信来源和店铺范围路由保护，但会直接读取店铺凭证并调用 `POST /open-api/goods/query-document-state`，未经过 `Erp07SheinAdapter` 的 source-pending 双重证据采集门禁。V2 批量发布页面的“批量回读”流程真实依赖该路由，不能将其误判为死代码。
...
## 56. ERP-07 网页读取、发品预检与旧本地入口 source-pending 绕过收口

### RUN-20260830-ERP07-SOURCE-PENDING-WEB-AND-LEGACY-GUARD-18

- 类型：ERP-07 本地隔离 fail-closed 修正；继续审计 `sales.sku`、`preflight.publish_quota` 和 `rules.custom_attribute_permission` 后，收口网页经营同步、网页发品预检、发布规则读取和复用共享函数的旧本地直连入口，不触碰生产。
- 失败基线：`syncStoreBusiness()` 会在读取凭据后，经 `syncStoreBusinessData()` 直连 `POST /open-api/goods/query-sku-sales`；`preflightPublish()` 会在读取凭据后，经 `runPublishPreflight()` 直连发布额度接口。发布规则读取在管理员首次加载含销售属性类目时也会直连自定义属性权限接口。前三者均未经过 source-pending adapter 双重证据采集门禁；旧 `server/index.js` 也复用了两个共享函数。
- 实际修正：云端网页服务的经营同步与发品预检现在无条件在凭据读取和网络传输前返回 `409 ERP07_ADAPTER_SOURCE_PENDING_READ_DISABLED`，没有运行时配置或构造器开关可解除。两个共享函数默认也在任何请求前拒绝，令旧本地直连入口同步受保护；只有测试文件用名称明确的 `allowSourcePendingSyntheticReadForTest=true` 保留离线 fixture 契约回归，生产/网页/Worker/路由均不传递该参数。发布规则读取保留已核验的类目/属性模板，但对当前与历史快照均返回空的自定义属性权限与受控诊断，不发送 source-pending 请求或写入权限快照。
- 失败回归：新增网页服务双入口零凭据/零传输断言，并为共享经营同步与预检各新增零请求断言；发布规则回归断言不请求自定义属性权限、不会写入该权限快照且返回 fail-closed 空权限。原有合成响应契约测试须显式标记测试用途。定向回归 `49/49` 通过。
- 全量验证：`npm test` 为 `1381/1381` 通过；V2 构建通过（`1953 modules transformed`）；Node `24.16.0`、npm `11.13.0` 工具链通过；密钥扫描 `scannedFiles=651, findings=[]`；静态 release audit `15/15`、staging isolation `14/14` 均通过。release audit 仍确认 `executionEnabled=false`、`authorizesPublishing=false`。
- 用户行为边界：若本地变更未来被部署，远端经营刷新和发品预检会如实得到稳定 409；发布规则页面不提供未经证据支持的自定义属性值授权，而不会读取店铺凭据、发送待核验读取、伪造销量或额度结论；已有本地缓存和历史投影不被回填、清除或改写。
- 调用范围复核：云端批量发布服务注入同一个 `webBusiness.preflightPublish()`，因此会得到相同的受控 409；`runProductRemotePreflight()` 与商品候选模块只消费预检结果、记录 endpoint 名称，不自行发起 SHEIN 请求。未发现额外网络旁路。
...
## 58. ERP-07 受控部署与线上只读 canary 收口

### RUN-20260831-ERP07-DEPLOYED-CANARY-22

- 类型：ERP-07 完成门审查与受控只读部署；用户已授权本次受控云端切换。仅部署经完整 manifest 审计的控制服务和网页制品；不迁移数据库、不修改共享配置、不打开业务写入、不重启 PostgreSQL、Redis、同步 Worker 或 Webhook。
- 候选与回退：源码 revision `474d3de165b16a0c81b2e38980be42e02cd5426b`；完整包 `shein-cloud-deploy-20260831-erp07-controlled-web-read-v2.tar.gz`，SHA-256 `af3fb6536b236668780d4e1864bbabeed70181f82c7195222c9f4139516f80cf`。旧 release 与原控制服务镜像均保留作为回退点。候选包不含真实 `.env`、Git 元数据、依赖目录或数据库数据。
- 发布前验证：adapter/网页服务/经营同步/预检 `68/68`，控制面与 ERP-07 相邻 `121/121`，全量 `npm test` `1406/1406`；V2 构建、固定 Node/npm 工具链、密钥扫描（`655` 文件零 findings）、release audit `15/15`、staging isolation `14/14`、release manifest 和完整包审计均通过。写入开关保持关闭。
- 部署过程：先在独立云端候选目录确认无 `._*` AppleDouble 文件、完整 manifest 与前端资源 hash 正确，再构建新 control 镜像；随后原子切换 `current` 并只重建 `control`。首次公网 `500` 被定位为新 release 根目录的 Nginx 穿透权限不足，未发生应用或数据库故障；将该 release 根目录校正为 `755` 后恢复。此教训已写入部署说明，后续不得省略候选目录与根目录权限检查。
- 部署后验证：控制服务 Docker health、`/health`、`/ready`、运行时数据库角色审计 `51` 项、公网页面 `200`、页面 SHA-256 与 release `dist-web/index.html` 一致、未登录 `app` 域网页会话 `401` 均通过。`api` 子域对非白名单 `/v1/*` 返回 `404` 是既有安全路由边界；网页的相对 `/v1/*` 请求由 `app` 域正确代理。
