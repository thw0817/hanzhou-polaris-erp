...
步骤：ERP-05  
状态：`BLOCKED`（缺少可安全读取的历史数据证据，不允许用猜测补齐）  
审计时间：2026-08-29（Asia/Shanghai）

## 1. 审计结论

本次 Run 已完成本地静态证据盘点，但未取得任何可逐条读取的历史业务记录。因此：

1. 不能宣称历史 Draft、Batch、Job、Run、Receipt、Review、SPU、SKC、SKU、媒体或队列记录已经完成 100% 分类。
2. 所有依赖实际数据行、实际队列消息、实际日志和实际对象存储清单的结论均标记为 `UNKNOWN`。
...
4. ERP-20 的修复范围目前只能精确到“结构性风险清单”，不能精确到历史记录 ID、租户、店铺、SKC、版本和 Attempt；因此 ERP-05 完成门未通过，ERP-06 不得开始。
...
以下项目没有本地快照或可安全读取的非生产证据，全部不能从历史文档推断：

- `product_drafts`、`publish_batches`、`publish_batch_items`、`publish_jobs`、`publish_execution_runs`、`publish_receipts` 的实际数据行；
...
| 范围 | 当前静态事实 | 历史记录分类 |
|---|---|---|
| `product_drafts` | 以 tenant/store 作用域保存 `draft_data`，状态约束含 `draft/blocked/ready/published/archived` | 实际行数与每行归属：`UNKNOWN`；模型存在 legacy 状态混用风险 |
...
| `media_assets` / references | 资产有用途、状态、sha256、引用计数；引用类型没有 `product_version` | 历史发布素材所有权：`UNKNOWN`；结构上存在版本边界缺口 |
...
由于没有实际数据快照，本节只登记可证实的“证据可得性”，不伪造记录数量。

| 记录集合 | mapped | legacy_unversioned | unmatched | conflict | UNKNOWN |
|---|---:|---:|---:|---:|---:|
| ProductDraft 实际行 | 0（未取得） | 0（未取得） | 0（未取得） | 0（未取得） | 全部实际行 |
| Batch / BatchItem 实际行 | 0（未取得） | 0（未取得） | 0（未取得） | 0（未取得） | 全部实际行 |
| Job / Run / Receipt 实际行 | 0（未取得） | 0（未取得） | 0（未取得） | 0（未取得） | 全部实际行 |
| Review / Webhook / 回读实际行 | 0（未取得） | 0（未取得） | 0（未取得） | 0（未取得） | 全部实际行 |
| SPU / SKC / SKU / Platform ID 实际行 | 0（未取得） | 0（未取得） | 0（未取得） | 0（未取得） | 全部实际行 |
| Media / Rule / Template 实际实例 | 0（未取得） | 0（未取得） | 0（未取得） | 0（未取得） | 全部实际实例 |
| Redis / Worker / 页面历史现场 | 0（未取得） | 0（未取得） | 0（未取得） | 0（未取得） | 全部实际现场 |

这里的 `0（未取得）` 表示“本次审计取得的可分类记录数为零”，不是业务数据库中记录数为零。业务记录总量均为 `UNKNOWN`。

## 7. ERP-05 必查项闭合情况

| 检查项 | 结果 | 说明 |
|---|---|---|
| tenant/store/SKC/version 关联 | `UNKNOWN` | 仅确认代码有部分作用域字段，未取得数据行 |
...
| 状态类别计数 | `UNKNOWN` | 无 DB snapshot |
| Draft 是否有执行记录 | `UNKNOWN` | 无 Draft/Job 行 |
...
| 媒体 released/needs 分类 | `UNKNOWN` | 无媒体对象和引用快照 |
...
在取得逐条数据前，ERP-20 只能登记以下“待证实修复族”，不能生成具体 UPDATE/DELETE 计划：
...
要使 ERP-05 重新进入可完成状态，需要一份脱敏、只读、可校验的证据包，至少包含：
